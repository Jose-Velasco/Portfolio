...
export class ProjectsService {
  public projectSelected = new BehaviorSubject<Project>(null);
  ...
  setProjects(dataOfProjects: any[]) {
    const projects: Project[] = [];
    dataOfProjects.forEach((project: Project) => {
      project = new Project(
        project.id,
        project.title,
        project.abstract,
  ...
  fetchProject() {
    return this.http.get<Project[]>(`${this._backendApiURL}/projects`, {headers: this._httpHeaders});
  }
  ...
  //   for (let index = 0; index < this._projects.value.length; index++) {
  //     if (this._projects[index].isFeatured) {
  ...
  get projectSelectedObserv(): Observable<Project> {
